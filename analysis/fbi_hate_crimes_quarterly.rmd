...
legend_points_fbi <-  tibble(quarter_date= dmy(c("01-10-2016", "01-10-2012", "01-10-2008", "01-07-2001")),
                         labels= c("Trump 2016", "Obama 2012", "Obama 2008", "9/11")) %>%
  left_join(qt_hate %>% 
...
qt_hate %>% 
  ...
  theme(plot.caption = element_text(size = 12))+
  theme(axis.text.x = element_text(angle=45, hjust = 1))+
  geom_point(data=legend_points_fbi, aes(x=quarter_date, y= hatedetrend , shape=factor(labels)), size=4,colour="grey50" )+
  theme(legend.position="bottom")+
...
```{r}
qt_hate_hispanic %>% View() 
  select(year, hatecrimes, residuals, )
  group_by(year) %>% 
  summarise( detrend_sum =sum(hatedetrend)) %>% View()
